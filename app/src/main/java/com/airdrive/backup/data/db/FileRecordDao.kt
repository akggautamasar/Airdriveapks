...
interface FileRecordDao {
    ...
    @Query("SELECT * FROM file_records WHERE (:query = '' OR displayName LIKE '%' || :query || '%' OR uri LIKE '%' || :query || '%') AND (:categoryName = '' OR category = :categoryName) AND (:statusName = '' OR status = :statusName) AND (:localStateName = '' OR localState = :localStateName) AND (:folder = '' OR uri LIKE '%' || :folder || '%') AND (:chatId = 0 OR destinationChannelId = :chatId) AND (:minBytes <= 0 OR sizeBytes >= :minBytes) AND (:maxBytes <= 0 OR sizeBytes <= :maxBytes) AND (:fromMillis <= 0 OR modifiedAtMillis >= :fromMillis) AND (:toMillis <= 0 OR modifiedAtMillis <= :toMillis) ORDER BY CASE WHEN :sort IN ('oldest','asc','modified_asc') THEN modifiedAtMillis END ASC, CASE WHEN :sort IN ('largest','size_desc') THEN sizeBytes END DESC, CASE WHEN :sort IN ('smallest','size_asc') THEN sizeBytes END ASC, CASE WHEN :sort IN ('name','name_asc') THEN displayName END COLLATE NOCASE ASC, modifiedAtMillis DESC, id DESC LIMIT :limit")
    fun searchFlow(query: String, categoryName: String, statusName: String, localStateName: String, folder: String, chatId: Long, minBytes: Long, maxBytes: Long, fromMillis: Long, toMillis: Long, sort: String, limit: Int): Flow<List<FileRecord>>

    @Query("SELECT COUNT(*) FROM file_records WHERE (:query = '' OR displayName LIKE '%' || :query || '%' OR uri LIKE '%' || :query || '%') AND (:categoryName = '' OR category = :categoryName) AND (:statusName = '' OR status = :statusName) AND (:localStateName = '' OR localState = :localStateName) AND (:folder = '' OR uri LIKE '%' || :folder || '%') AND (:chatId = 0 OR destinationChannelId = :chatId) AND (:minBytes <= 0 OR sizeBytes >= :minBytes) AND (:maxBytes <= 0 OR sizeBytes <= :maxBytes) AND (:fromMillis <= 0 OR modifiedAtMillis >= :fromMillis) AND (:toMillis <= 0 OR modifiedAtMillis <= :toMillis)")
