...
object Keys {
    ...
    /**
     * Deleted File Protection. AUTO_DELETE_MISSING_ENABLED defaults to **off** on purpose: the
     * whole point of the feature is that a file vanishing from the phone does not vanish from
     * Telegram, so AirDrive must never start throwing cloud copies away unless asked.
     */
...
class SettingsStore(private val context: Context) {
    ...
    /** Off unless the user turns it on; see [Keys.AUTO_DELETE_MISSING_ENABLED]. */
    val autoDeleteMissingEnabled: Flow<Boolean> =
        context.dataStore.data.map { it[Keys.AUTO_DELETE_MISSING_ENABLED] ?: false }

    /**
     * How long a file that vanished from the phone is kept in Telegram before an enabled
     * auto-delete sweep removes it. Clamped to at least a week so a stray tap cannot set
     * "delete tomorrow" on the user's only remaining copies.
     */
    val autoDeleteMissingDays: Flow<Long> = context.dataStore.data.map {
        (it[Keys.AUTO_DELETE_MISSING_DAYS] ?: DEFAULT_AUTO_DELETE_DAYS)
            .coerceIn(MIN_AUTO_DELETE_DAYS, MAX_AUTO_DELETE_DAYS)
    }

    suspend fun setAutoDeleteMissingEnabled(v: Boolean) {
        context.dataStore.edit { it[Keys.AUTO_DELETE_MISSING_ENABLED] = v }
    }

    suspend fun setAutoDeleteMissingDays(days: Long) {
        context.dataStore.edit {
            it[Keys.AUTO_DELETE_MISSING_DAYS] = days.coerceIn(MIN_AUTO_DELETE_DAYS, MAX_AUTO_DELETE_DAYS)
        }
    }
    ...
    suspend fun exportSummary(): String {
        ...
        return buildString {
            ...
            appendLine("auto_delete_missing=${autoDeleteMissingEnabled.first()}")
            appendLine("auto_delete_missing_days=${autoDeleteMissingDays.first()}")
        }
    }

    companion object {
        const val DEFAULT_AUTO_DELETE_DAYS = 30L
        const val MIN_AUTO_DELETE_DAYS = 7L
        const val MAX_AUTO_DELETE_DAYS = 365L
    }
}
